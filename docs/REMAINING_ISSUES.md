...
### ✅ 7차 수정 내역 — 코드·기능 리스트업(A/B/C) 후속
- **A-1/A-3** — 앱 실행 시 알림 재예약: `hasNotifPermission()`으로 권한 미허용이면 **조용히 스킵**(실행마다 실패 토스트 제거), 성공 시 정시 알람 안내(`checkExactAlarm`)까지 일원화 (`notifications.ts`/`App.tsx`)
- **A-2** — 일정 API 일 1회 캐시: `lib/sched-cache.ts`(team+날짜 키), 같은 날 재요청 없이 캐시 사용 → 비공식 API 호출 절감. 단위 테스트 6개 추가
- **C-3** — "🔄 갱신" 헤더 버튼: 자동 소스 전체를 지금 즉시 재적용(KBO 일일 이미지 강제 최신화), 진행 중 비활성화·부분 실패 집계
