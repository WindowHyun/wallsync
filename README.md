...
- **지금 적용** — 홈 / 잠금 / 둘 다 대상 선택 후 즉시 배경화면 변경 (화면별 "적용중" 상태 표시)
- **자동 갱신** — N시간마다 또는 매일 특정 시각에 백그라운드로 재적용 (WorkManager, 앱이 꺼져 있어도 동작)
- **실행 조건** — Wi-Fi(비과금 네트워크)에서만 / 충전 중에만 갱신하도록 제한 가능
- **자동 갱신 이력·실패 알림** — 각 배경의 마지막 자동 갱신 성공/실패를 카드에 표시하고, 실패 시 알림(Android 13+ 알림 권한)
- **백업/복원** — 등록 목록을 JSON으로 내보내기(클립보드·파일)·가져오기 (기기 이전 대비)
...
`main`에 push하거나 `main`으로 향하는 PR을 올리면(또는 Actions 탭에서 수동 실행하면)
GitHub Actions가 자동으로 Debug APK를 빌드합니다. 빌드 전 `npm run typecheck`로 타입 검사를 거칩니다.
...
- 자동 갱신 최소 간격은 안드로이드 정책상 15분이며, 배터리 절약/도즈 모드에서 실행이 지연될 수 있습니다.
  안정적으로 쓰려면 WallSync를 **배터리 최적화 예외**로 두세요. 앱 상단의 **"🔋 배터리 최적화 해제"
  배너 → 해제하기** 버튼으로 한 번에 설정할 수 있습니다(삼성 등은 "수동 설정"으로 해당 화면 이동).
